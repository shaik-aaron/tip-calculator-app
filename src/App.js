...
function App() {
  ...
  const tip_amount = ((bill * percentage) / 100 / Number(people)).toFixed(2);
  const total = ((bill + (bill * percentage) / 100) / Number(people)).toFixed(
    2
  );
      ...
      <div className="container">
        <form>
          ...
          <input
            ...
            placeholder="0"
          ></input>
          <label className="headings">Select Tip %</label>
          <div className="tip--percentages">
            <button
              type="button"
              onClick={() => setPercentage(5)}
              className={
                percentage === 5 ? "percentage--selected" : "percentage--values"
              }
            >
              5%
            </button>
          ...
          <div className="people--text">
            ...
            <p className={people === "0" ? "error" : "no--error"}>
              Can't be zero
            </p>
          </div>
          <input
            onChange={(event) => setPeople(event.target.value)}
            className={people === "0" ? "people--error" : "people"}
            placeholder="0"
          ></input>
        </form>
        <div className="result">
          <div className="tip--result">
            <div className="text">
              <p className="result--text">Tip Amount</p>
              <p className="result--subtext">/ person</p>
            </div>
            {isNaN(tip_amount) || tip_amount == Infinity ? (
              <p className="tip--amount">$0.00</p>
            ) : (
              <p className="tip--amount">${tip_amount}</p>
            )}
          </div>
          <div className="tip--result">
            <div className="text">
              <p className="result--text">Total</p>
              <p className="result--subtext">/ person</p>
            </div>
          ...
          <button
            onClick={reset}
            className={total > 0 ? "reset--active" : "reset"}
          >
            RESET
          </button>
        </div>
